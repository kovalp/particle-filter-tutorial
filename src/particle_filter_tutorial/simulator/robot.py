...
class Robot:
    ...
    def __init__(self, x: float, y: float, theta: float,
                 std_forward: float, std_turn: float, std_meas_distance: float, std_meas_angle: float) -> None:
        """
    ...
    def __repr__(self) -> str:
        """."""
        return f"R({self.x:.4f}, {self.y:.4f}, {self.theta:.4f})"

    def move(self, desired_distance: float, desired_rotation: float, world: World) -> None:
        """
    ...
    def measure(self, world: World) -> list[np.ndarray]:
        """
        ...
        for lm in world.landmarks:
            ...
            # Store measurement
            measurements.append(np.array([z_distance, z_angle]))

        return measurements

    @staticmethod
    def _get_gaussian_noise_sample(mu: float, sigma: float) -> float:
        """
